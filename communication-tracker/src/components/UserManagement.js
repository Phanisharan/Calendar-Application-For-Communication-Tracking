...
const UserManagement = ({ setShowNavbar }) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/user-module/users/')
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const url = isEdit ? `http://localhost:8000/user-module/users/${selectedUser.id}/` : 'http://localhost:8000/user-module/users/';
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8000/user-module/users/${id}/`)
